...
import static de.gematik.pki.gemlibpki.commons.tsl.TslUtils.tslDownloadUrlMatchesOid;
...
public final class TslModifier {
        ...
        .filter(
            tsp -> {
              final String tslTspName =
                  tsp.getTSPInformation().getTSPName().getName().getFirst().getValue();
  ...
  public static void setOtherTSLPointers(
    ...
    for (final Map.Entry<String, String> entry : tslPointerValues.entrySet()) {
      oTslPointers
          .getOtherTSLPointer()
          .addFirst(createOtherTSLPointerType(entry.getKey(), entry.getValue()));
    }
  ...
  private static List<MultiLangNormStringType> findTspTradeNames(
    ...
    final String nameTspName = tspInformation.getTSPName().getName().getFirst().getValue();

    final MultiLangNormStringType nameElement =
        tspInformation.getTSPTradeName().getName().getFirst();
